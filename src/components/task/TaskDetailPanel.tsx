...
import { useTask, useTasks, useUpdateTask, useDeleteTask, useArchiveTask, useUnarchiveTask } from "@/hooks/useTasks";
...
export function TaskDetailPanel({ taskId, projectId, onClose }: TaskDetailPanelProps) {
  const { project, columns, tags: projectTags, doneColumnIds, canEditTask, canDeleteTask, canArchiveTask } = useProjectContext();
  ...
  const handleTagsChange = (tagIds: string[]) => {
    const tags = projectTags.filter((t) => tagIds.includes(t.id)).map(({ id, name, slug, color }) => ({ id, name, slug, color }));
    setTaskTags.mutate({ taskId, tagIds, tags }, { onError: (err) => toast.error(err.message) });
  };
  ...
  const handleAssigneesChange = (assigneeIds: string[]) => {
    ...
    setTaskAssignees.mutate({ taskId, assigneeIds, assignees }, { onError: (err) => toast.error(err.message) });
  };

  const handleDependenciesChange: React.ComponentProps<typeof DependencySelect>["onChange"] = (depIds, deps) => {
    setTaskDependencies.mutate({ taskId, dependsOnIds: depIds, dependencies: deps }, { onError: (err) => toast.error(err.message) });
  };
  ...
  return (
    <Dialog open onClose={onClose} className="max-w-[calc(100vw-2rem)] sm:max-w-3xl max-h-[85vh] flex flex-col overflow-hidden">
          ...
          <div className="flex items-center gap-4 text-xs text-muted-foreground border-t border-border pt-3">
            {task.creator && (
              <div className="flex items-center gap-1.5">
                <Avatar name={task.creator?.full_name ?? "Deleted User"} url={task.creator?.avatar_url ?? null} size="sm" />
                <span>
                  Created by <span className="capitalize">{task.creator?.full_name ?? "Deleted User"}</span>
                </span>
              </div>
            )}
            <span>
      ...
      <ConfirmDialog open={showDeleteConfirm} onClose={() => setShowDeleteConfirm(false)} onConfirm={handleDelete} title="Delete Task" description="Delete this task? This action cannot be undone." confirmLabel="Delete" isPending={deleteTask.isPending} />
            ...
            <ul className="mt-2 space-y-1">
              {unfinishedDeps.map((d) => (
                <li key={d.id} className="flex items-center gap-1.5">
                  <span className="font-mono text-xs text-primary">{formatTaskRef(project.prefix, d.task_number)}</span>
